...
from parlai_internal.agents.seq2seq.torch_generator_agent2 import TorchGeneratorModel2
...
class Seq2seq(TorchGeneratorModel2):
    """
    ...
    def forward(self, *xs, ys=None, prev_enc=None, maxlen=None, bsz=None, history=None, context=None, nograd=False):
        ...
        if nograd:
            with torch.no_grad():
                ...
                context_states, context_hidden = self.context(encoder_states[0], context)
        
                self.prev_context = context_hidden

                scores, pred = self.decode_forced(encoder_states, ys, context_hidden)
        else:
        ...
        return scores, pred, context, encoder_states
...
class RNNContext(nn.Module):
    ...
    def __init__(
        ...
        self.context = nn.GRU(input_size=hiddensize,
    ...
    def forward(self, x, hidden):
        ...
        output, hidden = self.context(x, hidden)
...
class RNNDecoder(nn.Module):
    ...
    def forward(self, xs, encoder_output, context, incremental_state=None):
        """
        ...
        #newout = torch.cat((context, xes), -1)
        output, new_hidden = self.rnn(xes, context)
