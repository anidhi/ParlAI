...
class TorchGeneratorModel2(nn.Module, ABC):
    """
    ...
    def decode_forced(self, encoder_states, ys, context):
        ...
        latent, _ = self.decoder(inputs, encoder_states, context)
...
class TorchGeneratorAgent2(TorchAgent, ABC):
    """
    ...
    def add_cmdline_args(cls, argparser):
        ...
        super(TorchGeneratorAgent2, cls).add_cmdline_args(argparser)
        return agent
    ...
    def _init_cuda_buffer(self, batchsize, maxlen, force=False):
        ...
        if self.use_cuda and (force or not hasattr(self, 'buffer_initialized')):
            try:
                loss = self.compute_loss(self._dummy_batch(batchsize, maxlen),context=self.model.context.initHidden())
                self.backward(loss)
    ...
    def compute_loss(self, batch, context, return_output=False):
        """
        ...
        #print('jostpry', self.history.get_history_vec())
        
        #print('model input', batch.text_vec.shape)
        #print(batch
        model_output = None
        
        #Model normally does A->B C->D but need the B->C link
        if len(self.history.history_vecs) == 1:
            
            #A -> B
            model_output = self.model(*self._model_input(batch), ys=batch.label_vec, history=self.history.history_vecs,context=context)
            ...
            numpieces = len(self.history.history_vecs)
            startOfReal = numpieces - 2
            curIndex = 0
            counter = 0

            while counter != startOfReal:
                    
                src = self.history.history_vecs[counter]
                tgt = self.history.history_vecs[counter + 1]
                
                
                srcIn = batch.text_vec[0][curIndex:curIndex+len(src)].unsqueeze(0)
                curIndex = curIndex + len(src) + 1
                tgtIn = batch.text_vec[0][curIndex:curIndex+len(tgt)].unsqueeze(0)
                temp_out = self.model(*(srcIn,), ys=tgtIn, history=self.history.history_vecs, context=context,nograd=True)

                scores, preds, context, *_ = temp_out
                counter += 1
            ...
            #B->C
            #slice for C:
            Ctext = self.history.history_vecs[-1]
            Btext = self.history.history_vecs[-2]
            Cinput = batch.text_vec[0][batch.text_vec.shape[-1] - len(Ctext):].unsqueeze(0)
            Binput = batch.text_vec[0][batch.text_vec.shape[-1] - len(Btext) - len(Ctext)-1:].unsqueeze(0)

             

            BCout = self.model(*(Binput,), ys=Cinput, history=self.history.history_vecs,context=context)
            scores, preds, context, *_ = BCout

            score_view = scores.view(-1, scores.size(-1))
            BCloss = self.criterion(score_view, Cinput.view(-1)).sum()

            notnull = Cinput.ne(self.NULL_IDX)
            target_tokens = notnull.long().sum().item()
            correct = ((Cinput == preds) * notnull).sum().item()
            self.metrics['correct_tokens'] += correct
            self.metrics['nll_loss'] += BCloss.item()
            self.metrics['num_tokens'] += target_tokens
            BCloss /= target_tokens
            
            
            CDout = self.model(*(Cinput,), ys=batch.label_vec, history=self.history.history_vecs,context=context)
            scores, preds, context, *_ = CDout
            score_view = scores.view(-1, scores.size(-1))
            CDloss = self.criterion(score_view, batch.label_vec.view(-1)).sum()

            notnull = batch.label_vec.ne(self.NULL_IDX)
            target_tokens = notnull.long().sum().item()
            correct = ((batch.label_vec == preds) * notnull).sum().item()
            self.metrics['correct_tokens'] += correct
            self.metrics['nll_loss'] += CDloss.item()
            self.metrics['num_tokens'] += target_tokens
            CDloss /= target_tokens
            loss = (BCloss + CDloss) / 2
            if return_output:
                #doesn't work
                return (loss, CDout)
            else:
    ...
    def train_step(self, batch):
        ...
        self.model.train()
        context = self.model.context.initHidden()
        self.zero_grad()
        try:
